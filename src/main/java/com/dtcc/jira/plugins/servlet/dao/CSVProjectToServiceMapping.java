	...
	try {
		...
		if (StringUtils.isNotEmpty(nextLine[EXCEL_PROJECT_NAME_COLUMN])) {
			...
			if (StringUtils.equalsIgnoreCase(nextLine[position], "1")) {
			    ...
			    List<ProjectInfo> projectList = serviceNameToProjectListMap.get(serviceNames.get(position2));

			    if (projectList == null) {
				projectList = new ArrayList<ProjectInfo>();
			    }
			    projectList.add(
				    new ProjectInfo(nextLine[0], nextLine[EXCEL_PROJECT_NAME_COLUMN], nextLine[2]));
